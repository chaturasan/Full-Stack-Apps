...
signup_schema = Schema({
    'email': And(str, lambda s: len(s) >= 3 and re.match(EMAIL_REGEX_PATTERN, s), error='Invalid email'),
...
login_schema = Schema({
    'email': And(str, lambda s: len(s) >= 1 and re.match(EMAIL_REGEX_PATTERN, s), error='Invalid email'),
